                    ...
                    <div className={styles.menu_section}>
                        {<h2 className={styles.menu_title}>Menu</h2>}
                        {
                            MainMenu.filter((_, index) => index < 5 && index > 0).map((menu, index) => (
                                <Link key={menu.id} className={`${styles.menu_item} ${menu.link === section && styles.menu_itemCurrent} ${menu.name}`} onClick={() => setLoadingState(true)} href={`/${menu.link}${index > 0 ? "?page=1" : ""}`} title={`Ir a ${menu.name}`}>{menu.icon}{menu.name}</Link>
                            ))
                        }
                    </div>
                    {session.session_id &&
