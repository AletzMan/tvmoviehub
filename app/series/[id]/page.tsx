...
import { GetSerieCredits, GetSerieDetails, GetSerieRecommendations } from "@/app/services/fetchData"
...
export default async function Page(params: { params: { id: string }, searchParams: {} }) {
    ...
    const details: ISerieDetails = await GetSerieDetails(params.params.id)

    const credits: ICredits = await GetSerieCredits(params.params.id)

    const recommendation: IRecommendationResponse = await GetSerieRecommendations(params.params.id)
    ...
    return (
        <section className={`${styles.section} scrollBarStyle`}>
            ...
            {credits.cast.length > 0 && <SliderCrew credits={credits} type="cast" title="REPARTO" />}
            <Crew credits={credits} />
            <hr className="separator" />
            <MovieSlider parts={recommendation.results} title="RECOMENDACIONES" />
        </section>
    )
}